...
| 種別 | 正本（v3 草案） |
|------|----------------|
| API · route · auth | [`02-設計/features/03-新規登録/詳細設計-v3.md`](../02-設計/features/03-新規登録/詳細設計-v3.md) §3 |
| データ契約 · schema | 同上 §2 |
| retrofit · gap | 同上 §7 |
| テスト追跡 | [`03-テスト計画/features/03-新規登録/`](../03-テスト計画/features/03-新規登録/) DOC-REMED TC 節 |

> 設計詳細（オンボーディング HTTP 契約 · handle 検証 · legacy 互換経路）は **詳細設計 v3** へ移行済（2026-07）。本 REQ は What のみ。マジックリンク入口は **#01 ログイン** 正本。
...
| `@ユーザーID` の空きを確認する | オンボーディング入力中（デバウンス 500ms） | handle 空きチェック（→ **詳細設計 v3 §7.2** · legacy salvage） |
...
- JWT 保持中の **初回プロフィール一括確定**（→ **詳細設計 v3 §3.1**）
...
- **FR-REG-01** 未登録メールでマジックリンクを発行すると、トークン発行時点で `isNewUser: true` が返る（`email_index` 未存在判定）。（→ **#01 ログイン 詳細設計 v3 §3.1**）
- **FR-REG-02** 有効トークン検証時、メールが未索引なら `user-{timestamp}-{random}` 形式の `userId` で `users/{userId}.json` を新規作成する。（→ **#01 ログイン 詳細設計 v3 §3.2**）
...
- **FR-REG-12** 初回プロフィール一括確定 API は認証必須。body: `handle`, `display_name`, `country`, `locale`, `timezone`, `theme`。（→ **詳細設計 v3 §3.1** · IHL: `onboarding/complete`）
...
- **FR-REG-16** handle 空きチェック API は認証不要。`{ ok: true, available: boolean }` を返す。（→ **詳細設計 v3 §7.2** · legacy salvage · IHL 未配線）

### レガシー経路（API のみ・分割 UI なし）

- **FR-REG-17** レガシー register — username / display_name 更新（ステップ分割時代の互換）。（→ **詳細設計 v3 §3.3** · legacy salvage）
- **FR-REG-18** レガシー locale — 言語のみ更新。（→ **詳細設計 v3 §7.2** · legacy salvage）
- **FR-REG-19** レガシー complete-onboarding — `agree_terms` / `agree_privacy` 必須。未同意は `MUST_AGREE_TO_TERMS`。（→ **詳細設計 v3 §3.3** · #02 連携）
...
| ドメインロジック | → **詳細設計 v3 §7.1** · `design/phases/Phase_auth_login.md` |
| オンボーディング UI | UI 設計 · `Phase0_入口_auth.md` |
| ルートゲート | 遷移設計 v1 · `ProtectedApp` 境界 |
| ログイン入口（登録トリガ） | #01 ログイン REQ · **詳細設計 v3 §3.1–3.2** |
...
3. **詳細設計 v3 §3** + UI 設計（オンボーディングゲート実態）
4. **詳細設計 v3 §7.1**（legacy salvage: verifyMagicLink · setupProfile · checkHandle）
5. **詳細設計 v3 §3.1–3.3**（onboarding complete/status/register）
...
| 2026-07-05 | DOC-REMED REQ-slim — API 経路を詳細設計 v3 §3 stub へ移行 · pattern 13→0 |
