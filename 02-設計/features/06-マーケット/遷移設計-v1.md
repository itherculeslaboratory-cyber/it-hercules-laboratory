...
| 3 評価確定 | **取引成立** | 手動評価 or 1ヶ月無評価で自動「良い」| **システム維持費税**（売上×8%）義務発生 → 未払いは fee_unpaid（§11.7 · ADR-H-38 §6.7）|
...
Stage 3 取引成立(§11.0.1)
  → trade-events 確定イベント
  → システム維持費税クロック起点(§11.7 · ADR-H-38 §6.7)
     計上: system_maintenance_tax_accrued · due_at = completed_at + 30d
     未払い継続 → 月次 Fibonacci Δcount(§08 §5.1)
  → 振込導線(§23 GMO · deriveTransferCode)
```

- **システム維持費税（売上×8%）はマッチング確定・settled・sold 単独では発生しない**（FR-MKT-13 を遷移で固定）。
