...
|---|---|---|---|
| 1 | 個人事業主口座での直接API契約可否(最重要・公式資料間の不整合) | 維持(未解消・銀行への直接確認が必要な状態も同一)。提供概要ページは同日時点も「本番環境において当社APIをご利用いただけるのは法人のお客さまに限定」と明記、個人事業主の扱いに個別記載なし。矛盾の構図に変化なし。 | https://gmo-aozora.com/business/api-cooperation/provisionpolicy.html (WebFetch再取得) — **C0対象外(人間ゲート/銀行への直接確認)** |
| 2 | レート制限の実数(接続通知書でのみ確定) | 対象外。公開資料の記載方針(429返却・詳細は接続通知書)は本レポート記載と一致し、公開ページに数値追加なし。 | 元調査PDF記載の踏襲確認のみ(再取得なし)。**C0対象外(契約時にしか確定しない・人間ゲート後)** |
| 3 | 接続審査の所要・要件 | 対象外。審査基準非公開の状態に変化なし(申込ページ構造も従来通り)。 | https://gmo-aozora.com/business/api-cooperation/provisionpolicy.html 内リンク構成確認 — **C0対象外(申込・審査は人間ゲート)** |
| 4 | sunabarと本番の差分(ホスト名・トークン取得・失効条件) | 維持。sunabarガイドを再取得し、審査なし・個人/法人/個人事業主とも利用可・トークン失効「ポータル最終ログインから30日超」を同日時点で再確認。元調査の記載と一致。 | https://gmo-aozora.com/sunabar/tutorial/01.html (WebFetch再取得) |
| 5 | 仕様書バージョン更新(実装直前に最新版PDF再取得の指示) | 対象外(実装直前作業として維持、本日の軽量確認では新版有無まで踏み込まず)。無償/有償API一覧ページの記載内容(振込入金明細照会・VA発行・イベント通知が無償側)は同日時点で一致確認。 | https://gmo-aozora.com/business/api-cooperation/apilineup.html (WebFetch再取得) |
| 6 | Webhook運用の落とし穴(配信停止→14日で削除、自己修復ループ設計) | 対象外(設計時点の実装事項であり、本日追加確認なし。仕様変更を示す情報も未検出)。 | 元調査PDF記載の踏襲確認のみ(再取得なし) |
| 7 | 手数料・料金改定(他行宛130円、口座維持手数料「記載なし」) | 維持。手数料ページを再取得し、他行宛振込130円(税込)/件、口座維持手数料の明記なしを同日時点で再確認。 | https://gmo-aozora.com/business/contents/fee.html (WebFetch再取得) |

**判定**: 選定前提は維持。今回再取得した4件(sunabar無料・審査なし、無償/有償API区分、法人限定の本番条件、手数料)はいずれも元調査の記載と一致し、崩れなし。銀行への直接確認・接続通知書取得・実キー疎通・仕様書最新版の実装直前再取得を要する項目(#1〜3, #5後半, #6)はC0の対象外(人間ゲートまたは実装直前作業)として引き続き未解消のまま維持。
