...
|---|---|---|---|
| 1 | YouTube Data API クォータ体系 | **軽微差分あり(選定は不変)**。videos.insert は 2025-12-04 に 1600 units→約100 units/回へ改定済みで、元調査の「100回/日の専用枠」記述は現状と一致。ただし 2026-05 頃から公式クォータとは別に "Video Uploads per day" という未文書化の隠れ上限で 429 が発生する報告が出ている(新情報)。未監査プロジェクトの private 固定は公式リファレンス通り変わらず。 | WebSearch: developers.google.com/youtube/v3/docs/videos/insert / github.com/googleapis/google-api-python-client issue #2753 |
| 2 | VOICEVOX キャラクター規約 | **変化なし**。ソフトウェア利用規約(商用・非商用ともクレジット表記で無料利用可)は維持。クレジット表記なし商用利用は1キャラ40万円/契約という有償ルートが存在することを追加確認(選定前提には影響なし、キャラ確定時に個別規約要確認の方針も維持)。 | WebSearch: voicevox.hiroshiba.jp/term/ 参照の複数解説記事(2025-12〜2026-05時点) |
| 3 | AivisSpeech のモデルライセンス | **変化なし**。AIVM形式・ACML/ACML-NC/CC0混在のAivisHub運用は元調査どおり。ACML 1.0は個人利用・商用利用・動画制作・ゲーム開発等を許容する実務的ライセンスと確認。モデル単位の商用可否確認方針は維持。 | WebSearch: github.com/Aivis-Project/ACML, aivis-project.com |
| 4 | X API 無料枠 | **崩れ(選定=半自動投稿の方針自体は不変)**。2026-02-06 に無料/Basic/Proのティア制が廃止され、新規開発者は pay-per-usage のクレジット制がデフォルトに変更済み(既存無料枠ユーザーは$10クーポン付きで移行)。500 post/月は「旧・無料枠」の過去の話となり、現行は従量課金(500post/月相当で約$7.50)。元調査の根拠21(pay-per-usage記載への変化)がすでに正しく予兆していた内容が2026-02に確定した形。**§6-4の指示どおり着手時再確認が必要という結論は維持されるが、「無料枠」という前提語彙自体を「有償・少額の従量課金」に修正する必要あり。** | WebSearch: postproxy.dev/blog/x-api-pricing-2026, socialcrawl.dev/blog/x-twitter-api-2026 |
| 5 | TikTok アプリ監査 | **変化なし**。未監査クライアントはSELF_ONLY固定・24時間5ユーザー上限・監査必須という制約は元調査と一致。日次上限目安15post/日も確認。 | WebSearch: developers.tiktok.com/doc/content-posting-api-get-started, content-sharing-guidelines |
| 6 | Style-Bert-VITS2 の AGPL | C0対象外(軽量確認範囲では新情報なし。ライセンス種別自体はGitHubリポジトリのLICENSE表記であり短期間で変わる性質のものではないため、着手時の法的整理という元調査の方針を維持) | 未実施(低優先度と判断・変更の兆候なし) |
| 7 | 8GB VRAM の実測 | C0対象外(人間ゲート/実機実測前提。対象GPU上での実測値でしか判定できずWeb確認になじまない) | 実測待ち(元調査どおり) |
| 8 | ffmpeg の libass 有効化 | C0対象外(導入時に `ffmpeg -version` で個体確認する運用手順であり、事前のWeb再検証になじまない) | 実装時のローカル確認手順のまま維持 |

**判定**: partially-changed — TTS(VOICEVOX/AivisSpeech)・合成(ffmpeg)・画像(ComfyUI/CLIP)・YouTube投稿・TikTok監査の選定前提はすべて維持。X API のみ「無料枠500post/月」という前提語彙が2026-02のティア制廃止により実態と乖離しており、§6-4の表現を「pay-per-usageの少額従量課金(500post/月で約$7.50相当)」へ改訂すべき。ただしX/TikTokはいずれも既に「人間が投稿する半自動」設計のため、B2の選定(投稿段=YouTubeのみ自動・TikTok/X半自動)自体への影響はなし。加えてYouTube側で未文書化の日次アップロード上限(2026-05以降報告)が新たに浮上しており、B5実装時に実プロジェクトでの挙動確認を強めに行う必要がある(§6-1の「実プロジェクトでの挙動も着手時に確認」という既存文言で対応範囲内)。
